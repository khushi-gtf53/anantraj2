  ...
  return (
    <section className="relative px-[20px] lg:px-[100px] py-[40px] lg:py-[100px] bg-white">
      <div className="relative z-[3] mx-auto">
        ...
        {/* Year Navigation */}
        <div
          className="flex flex-nowrap overflow-x-auto items-center text-[35px] lg:text-[40px] tracking-[1px] pt-5 mb-6 text-primaryblue font-sangbleu border-t border-black whitespace-nowrap scrollbar-hide"
            ...
            {slides.map((slide) => (
              <SwiperSlide key={slide.year}>
                <div className="flex flex-col md:flex-row items-start md:items-center flex-wrap gap-6">
                  <div className="w-full md:w-[55%] relative h-[200px] md:h-[350px]">
                    <Image
                      src={slide.image}
                      alt={`Timeline ${slide.year}`}
                      fill
                      className="object-cover w-full h-full"
                      priority={slide.year === "1969"} // preload first slide for LCP
                    />
                  </div>
                  <div className="w-full lg:text-left text-center md:w-[30%] md:ml-[60px] font-lato">
                    ...
                    <p className="my-4 tracking-[1px] leading-[26px] text-sm md:text-base">{slide.text}</p>
                    <div className="flex lg:justify-start justify-center items-center gap-2 mt-4">
                      <button onClick={() => swiperRef.current?.slidePrev()} className="cursor-pointer swiper-prev-timeline rotate-180" ref={prevRef}>
                        <Image src="/assets/right-arrow.png" alt="Previous" width={20} height={20} />
                      </button>
                      <button onClick={() => swiperRef.current?.slideNext()} className="cursor-pointer swiper-next-timeline" ref={nextRef}>
